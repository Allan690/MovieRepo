.PHONY: help
PROJECT_NAME ?= movie-backend
DOCKER_DEV_COMPOSE_FILE := "docker-compose.yaml"
TARGET_MAX_CHAR_NUM=10

# a variable that stores application's container id if the container is running
CONTAINER_ID := $(shell docker-compose ps -q app)
ifeq ($(CONTAINER_ID),)
	CONTAINER := $(shell docker-compose -f docker-compose.yaml ps -q app)
else
	CONTAINER := $(shell docker ps -q --no-trunc | grep $$(docker-compose -f docker-compose.yaml ps -q app))
endif
...
start:
	...
	@ docker volume create --name=movie_data > /dev/null
	@ echo "  "
...
ifeq ($(CONTAINER),)
	...
	${INFO} "Running movie database rollback"
	@docker-compose -f $(DOCKER_DEV_COMPOSE_FILE) exec app yarn roll:migrate development
